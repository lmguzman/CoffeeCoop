...
do.accounts <- function(force.git=FALSE, force.empty=FALSE) {
  ## checks to see if all changes have been committed to git.
  ## can't get this to work yet on my computer
  if ( !git.is.clean() )
    if ( force.git )
      warning("Directory not clean, but proceeding anyway")
    else
      stop("Directory not clean: use force.git=TRUE or git commit")
  ## saves date as character string
  today <- as.character(Sys.Date())
  ## makes path to file where results will be stored
  f.out <- sprintf("data/%s.csv", today)
  ## checks to see if this file is already there.
